...
import RevolutPaymentModal, { checkRevolutHealth } from './RevolutPaymentModal'
...
const SupplierInvoicesPage = ({ selectedCompany }) => {
  ...
  const handleOpenRevolutPay = async (invoice) => {
    const { canPay, warning } = await checkRevolutHealth()
    if (!canPay) {
      toast.error(warning || 'Revolut est hors ligne')
      return
    }
    if (warning) {
      toast(warning, { icon: '⚠️' })
    }
    setRevolutPayInvoice(invoice)
  }
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-center">
                          <ActionsDropdown
                            ...
                            onPay={handlePay}
                            onPayRevolut={handleOpenRevolutPay}
