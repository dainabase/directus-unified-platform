# PROMPT 6/8 - ENDPOINTS API FINANCE

## Contexte
Ce fichier expose tous les services Finance via des endpoints REST. Il utilise Express et connecte les 5 services créés précédemment au frontend React.

## Fichier à créer
`src/backend/api/finance/finance.routes.js`

## Dépendances requises
- Express.js
- Les 5 services Finance créés dans les prompts 1-5
...
`src/backend/api/finance/index.js`
...
// Monter les routes Finance
app.use('/api/finance', financeRoutes);
```

## Instructions pour Claude Code
1. Créer le fichier `src/backend/api/finance/finance.routes.js`
2. Créer le fichier `src/backend/api/finance/index.js`
3. Modifier `src/backend/server.js` pour ajouter les routes
4. Installer multer si pas déjà fait: `npm install multer`
...
# KPIs
curl http://localhost:3000/api/finance/kpis/HYPERVISUAL
...
curl -X POST http://localhost:3000/api/finance/invoices \
  ...
  -d '{
    ...
    "line_items": [
      {"description": "Service web", "quantity": 1, "unit_price": 5000}
    ]
  }'
...
curl -X POST http://localhost:3000/api/finance/ocr/process \
  ...
  -F "document_type=supplier_invoice" \
