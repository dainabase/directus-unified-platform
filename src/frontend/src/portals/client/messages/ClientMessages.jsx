            ...
            messages.map(msg => (
              <div key={msg.id} className={`flex ${isFromClient(msg) ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                  ...
                  {!isFromClient(msg) && (
                    <p className="text-xs font-medium mb-1" style={{ color: 'var(--accent, #0071E3)' }}>HYPERVISUAL</p>
                  )}
                  <p className="text-sm whitespace-pre-wrap">{msg.description}</p>
                  <p className={`text-xs mt-1 ${isFromClient(msg) ? 'text-white/60' : 'text-gray-400'}`}>
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-gray-100">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-400 text-sm"
            />
            <button
              type="submit"
              disabled={!newMessage.trim() || sendMutation.isPending}
              className="p-2.5 rounded-xl text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors" style={{ backgroundColor: 'var(--accent, #0071E3)' }}
            >
