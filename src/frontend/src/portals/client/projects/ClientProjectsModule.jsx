...
const STATUS_CONFIG = {
  active: { label: 'Actif', bg: 'var(--tint-green)', fg: 'var(--semantic-green)', icon: CheckCircle2 },
  in_progress: { label: 'En cours', bg: 'var(--accent-light)', fg: 'var(--accent)', icon: BarChart3 },
  on_hold: { label: 'En pause', bg: 'var(--tint-orange)', fg: 'var(--semantic-orange)', icon: PauseCircle },
  completed: { label: 'Termine', bg: 'var(--accent-light)', fg: 'var(--accent)', icon: CheckCircle2 },
  cancelled: { label: 'Annule', bg: 'var(--tint-red)', fg: 'var(--semantic-red)', icon: XCircle },
  planning: { label: 'Planification', bg: 'rgba(0,0,0,0.04)', fg: 'var(--label-2)', icon: Clock }
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <ChevronRight size={14} style={{color:'var(--label-3)'}} />
        <span className="text-sm font-medium" style={{color:'var(--label-1)'}}>{project.name}</span>
      </div>

      {/* Header */}
      <div className="ds-card p-6">
        <h2 className="text-xl font-bold" style={{color:'var(--label-1)'}}>{project.name}</h2>
        {project.description && <p className="text-sm mt-1" style={{color:'var(--label-3)'}}>{project.description}</p>}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
          <div>
            <p className="text-xs" style={{color:'var(--label-3)'}}>Debut</p>
            <p className="font-semibold" style={{color:'var(--label-1)'}}>{project.start_date ? format(new Date(project.start_date), 'dd MMM yyyy', { locale: fr }) : '—'}</p>
          </div>
          <div>
            <p className="text-xs" style={{color:'var(--label-3)'}}>Fin prevue</p>
            <p className="font-semibold" style={{color:'var(--label-1)'}}>{project.end_date ? format(new Date(project.end_date), 'dd MMM yyyy', { locale: fr }) : '—'}</p>
          </div>
          <div>
            <p className="text-xs" style={{color:'var(--label-3)'}}>Avancement</p>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold" style={{color:'var(--label-1)'}}>{progress}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="ds-card">
        <div className="p-4" style={{borderBottom:'1px solid var(--sep)'}}>
          <h3 className="font-semibold" style={{color:'var(--label-1)'}}>Livrables ({totalDel})</h3>
        </div>
        {isLoading ? (
          <div className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto" style={{color:'var(--accent)'}} /></div>
        ) : deliverables.length === 0 ? (
          <div className="p-8 text-center text-sm" style={{color:'var(--label-3)'}}>Aucun livrable pour ce projet</div>
        ) : (
            ...
            {deliverables.map(d => (
              <div key={d.id} className="px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm" style={{color:'var(--label-1)'}}>{d.title}</span>
                </div>
                {d.due_date && (
                  <span className="text-xs" style={{color: new Date(d.due_date) < new Date() && d.status !== 'done' ? 'var(--semantic-red)' : 'var(--label-3)'}}>
      ...
      {timeline.length > 0 && (
        <div className="ds-card">
          <div className="p-4" style={{borderBottom:'1px solid var(--sep)'}}>
            <h3 className="font-semibold" style={{color:'var(--label-1)'}}>Timeline</h3>
          </div>
          <div className="p-4">
            <div className="relative pl-6 space-y-4">
              <div className="absolute left-2 top-2 bottom-2 w-0.5" style={{background:'var(--sep)'}} />
              {timeline.map(d => (
                <div key={d.id} className="relative">
                  <div className="absolute -left-4 w-3 h-3 rounded-full border-2 border-white" style={{background: d.status === 'done' ? 'var(--semantic-green)' : '#D1D5DB'}} />
                  <div className="flex items-center justify-between">
                    <span className="text-sm" style={{color:'var(--label-2)'}}>{d.title}</span>
                    <span className="text-xs" style={{color:'var(--label-3)'}}>{format(new Date(d.date_created), 'dd MMM yyyy', { locale: fr })}</span>
                  </div>
                </div>
              ))}
            </div>
...
const ClientProjectsModule = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold" style={{color:'var(--label-1)'}}>Mes projets</h1>
        <p className="text-sm mt-0.5" style={{color:'var(--label-3)'}}>Suivi de vos projets en cours</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="ds-card p-12 text-center">
          <FolderKanban className="w-10 h-10 mx-auto mb-3" style={{color:'var(--label-3)'}} />
          <p style={{color:'var(--label-3)'}}>Aucun projet pour le moment</p>
        </div>
      ) : (
          ...
          {projects.map(p => (
            <div key={p.id} onClick={() => setSelectedProject(p)}
              ...
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-semibold" style={{color:'var(--label-1)'}}>{p.name}</h3>
                <StatusBadge status={p.status} />
              </div>
              {p.description && <p className="text-xs mb-3 line-clamp-2" style={{color:'var(--label-3)'}}>{p.description}</p>}
              <div className="flex items-center gap-4 text-xs" style={{color:'var(--label-3)'}}>
                {p.start_date && (
