...
class FinanceDashboardService {
  ...
  async getCashPosition(companyName) {
    ...
    try {
      ...
      const accounts = await directus.items('bank_accounts').readMany({
        filter: {
  ...
  async getReceivables(companyName) {
    ...
    try {
      const invoices = await directus.items('client_invoices').readMany({
        filter: {
          owner_company: { _eq: companyName },
  ...
  async getMonthlyExpenses(companyName, startDate, endDate) {
    ...
    try {
      const [supplierInvoices, expenses] = await Promise.all([
        directus.items('supplier_invoices').readMany({
          filter: {
            owner_company: { _eq: companyName },
            status: { _eq: 'paid' },
        ...
        directus.items('expenses').readMany({
          filter: {
            owner_company: { _eq: companyName },
  ...
  async getAlerts(companyName) {
    ...
    try {
      ...
      const overdueReceivables = await directus.items('client_invoices').readMany({
        filter: {
          owner_company: { _eq: companyName },
      ...
      const upcomingPayables = await directus.items('supplier_invoices').readMany({
        filter: {
          owner_company: { _eq: companyName },
      ...
      const unreconciledTx = await directus.items('bank_transactions').readMany({
        filter: {
          owner_company: { _eq: companyName },
