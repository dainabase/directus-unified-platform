            ...
            messages.map(msg => (
              <div key={msg.id} className={`flex ${isFromClient(msg) ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                  ...
                  <p className={`text-xs mt-1 ${isFromClient(msg) ? 'text-blue-200' : 'text-gray-400'}`}>
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-gray-100">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
            />
            <button
              type="submit"
              disabled={!newMessage.trim() || sendMutation.isPending}
              className="p-2.5 rounded-xl bg-[#0071E3] text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
