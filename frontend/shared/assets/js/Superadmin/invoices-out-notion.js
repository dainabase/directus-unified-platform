...
// Génération automatique, templates par entité, envoi email et suivi
...
window.InvoicesOutNotion = (function() {
    ...
    const config = {
        ...
        entityTemplates: {
            hypervisual: {
                ...
                color: '#e41e26',
                prefix: 'HYP',
                footer: 'Hypervisual Sàrl - IDE: CHE-123.456.789 TVA - Capital: 20\'000 CHF',
                bank: {
            ...
            waveform: {
                name: 'Waveform Studio Sàrl',
                logo: '/assets/img/logo-waveform.png',
                color: '#f76707',
                prefix: 'WAV',
                footer: 'Waveform Studio Sàrl - IDE: CHE-345.678.901 TVA - Capital: 20\'000 CHF',
                bank: {
        ...
        // Délais relances
        reminderDelays: {
            first: 7,   // 7 jours après échéance
            second: 14, // 14 jours après échéance
    ...
    async function init() {
        try {
            console.log('📤 Initialisation module factures clients...');
            ...
            console.log('✅ Module factures clients initialisé');
            
        } catch (error) {
            console.error('❌ Erreur initialisation factures clients:', error);
    ...
    async function loadInvoices() {
        try {
            ...
            // Configuration de la base de données (utilise la DB existante pour devis et factures)
            const DB_DEVIS_FACTURES = "226adb95-3c6f-8011-a9bb-ca31f7da8e6a"; // DB-DEVIS & FACTURES
            
            console.log('💼 Chargement des factures clients depuis Notion DB-DEVIS & FACTURES...');

            // Requête Notion avec filtrage sur les factures (pas les devis)
            ...
            state.invoices = response.results.map(page => {
                ...
                // Si relation client disponible, on utiliserait une requête pour récupérer les détails
                if (props["Client"]?.relation?.length > 0) {
                    client.company_name = "Client à résoudre";
                }

                // Extraire l'entité depuis la relation Projet
                const entity = extractEntityFromProject(props["Projet"]);

                // Générer ID facture basé sur l'entité et Notion ID
                const invoiceNumber = props["Référence"]?.rich_text?.[0]?.plain_text || 
                ...
                return {
                    ...
                    entity: entity,
                    client: client,
                    invoice_date: props["Date Émission"]?.date?.start || new Date().toISOString().split('T')[0],
                    due_date: props["Date Échéance"]?.date?.start || calculateDueDate(props["Date Émission"]?.date?.start),
            ...
            console.log(`✅ ${state.invoices.length} factures clients chargées depuis Notion DB-DEVIS & FACTURES`);
    ...
    function extractEntityFromProject(projetProperty) {
        ...
        // Dans un vrai système, on résoudrait la relation pour obtenir l'entité
    ...
    function buildInvoiceItems(descriptionProperty, totalHT) {
        ...
        // Pour la démo, créer un item unique. Dans un vrai système, on parserait ou récupérerait les items
        return [
            {
    ...
    function buildInvoiceTracking(props, status) {
        ...
        if (status === "viewed" || status === "paid") {
            ...
            tracking.opened_date = props["Date Émission"]?.date?.start ? 
                addDaysToDate(props["Date Émission"].date.start, 1) : null;
            tracking.last_activity = tracking.opened_date;
        }

        if (status === "paid") {
            tracking.paid_date = props["Date Échéance"]?.date?.start ? 
                subtractDaysFromDate(props["Date Échéance"].date.start, 2) : null;
        }

        if (status === "overdue") {
            tracking.reminders = [
                {
                    date: addDaysToDate(props["Date Échéance"]?.date?.start || new Date().toISOString(), 7),
                    type: "email",
                    status: "sent"
                }
            ];
        ...
        state.invoices = [
                {
                    ...
                    items: [
                        {
                            description: "Production vidéo corporate 4K",
                            quantity: 1,
                            unit_price: 10000.00,
                            vat_rate: 7.7,
                            total_ht: 10000.00
                        ...
                        {
                            description: "Motion design logo animé",
                            quantity: 2,
                            unit_price: 1250.00,
                            vat_rate: 7.7,
                            total_ht: 2500.00
                ...
                },
                {
                    ...
                    items: [
                        {
                            description: "Conseil stratégie digitale",
                            quantity: 40,
                            unit_price: 300.00,
                            vat_rate: 7.7,
                            total_ht: 12000.00
            ...
            console.log(`💼 ${state.invoices.length} factures clients de démonstration chargées`);
        }
        
        // Calculer prochains numéros
        calculateNextNumbers();
        
        console.log(`📋 ${state.invoices.length} factures clients chargées`);
    }
    ...
    // Calculer prochains numéros de facture
    function calculateNextNumbers() {
        Object.keys(config.entityTemplates).forEach(entity => {
            ...
            // Trouver le plus grand numéro pour cette entité cette année
    ...
    // Ouvrir modal sélection projet
    function openProjectSelectionModal() {
        // TODO: Implémenter modal sélection projet
        showNotification('Sélection depuis projet - À implémenter', 'info');
    }
    ...
    function searchClientData() {
        ...
        if (existingClient) {
            ...
            showNotification(`Client trouvé: ${existingClient.company_name}`, 'success');
        } else {
            showNotification('Client non trouvé dans la base', 'warning');
        }
    }
    
    // Mettre à jour données entité
    function updateEntityData() {
        const entity = document.getElementById('invoice-entity').value;
        if (!entity) return;
        
        const template = config.entityTemplates[entity];
        // Ici on pourrait pré-remplir des champs selon l'entité sélectionnée
        
        console.log(`Entité sélectionnée: ${template.name}`);
    }
    ...
    function previewInvoice() {
        ...
        // Fermer modal création et ouvrir aperçu
    ...
    function collectInvoiceData() {
        ...
        if (items.length === 0) {
            showNotification('Veuillez ajouter au moins une ligne à la facture', 'error');
            return null;
        }
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <div>
                            ...
                            ${invoice.client.contact_name ? `À l'attention de ${invoice.client.contact_name}<br>` : ''}
                ...
                <div class="row mt-4">
                    <div class="col-md-6">
                        ...
                        <div>
                            ...
                            Référence: ${invoice.qr_reference}
                        </div>
                    </div>
                    <div class="col-md-6 text-end">
    ...
    async function createInvoice() {
        try {
            if (!state.currentInvoice) {
                showNotification('Erreur: données de facture manquantes', 'error');
                return;
            }
            ...
            showNotification(`Facture ${state.currentInvoice.invoice_id} créée avec succès`, 'success');
    ...
    async function sendInvoiceByEmail() {
        try {
            ...
            // Créer d'abord la facture si elle n'existe pas
            ...
            showNotification(`Facture ${state.currentInvoice.invoice_id} envoyée à ${state.currentInvoice.client.email}`, 'success');
        ...
        return `
            <div class="row">
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        <tr><td><strong>Numéro:</strong></td><td>${invoice.invoice_id}</td></tr>
                        <tr><td><strong>Entité:</strong></td><td>${template.name}</td></tr>
                        <tr><td><strong>Date:</strong></td><td>${formatDate(invoice.invoice_date)}</td></tr>
                        <tr><td><strong>Échéance:</strong></td><td>${formatDate(invoice.due_date)}</td></tr>
            ...
            <div class="row mt-4">
                ...
                <div class="col-md-6">
                    <h4>Suivi</h4>
                    <table class="table table-sm">
                        ${invoice.sent_date ? `<tr><td>Envoyée:</td><td>${formatDateTime(invoice.sent_date)}</td></tr>` : ''}
                        ${invoice.opened_date ? `<tr><td>Ouverte:</td><td>${formatDateTime(invoice.opened_date)}</td></tr>` : ''}
                        ${invoice.paid_date ? `<tr><td>Payée:</td><td>${formatDateTime(invoice.paid_date)}</td></tr>` : ''}
                        <tr><td>Ouvertures:</td><td>${invoice.tracking?.opens || 0}</td></tr>
                        <tr><td>Téléchargements:</td><td>${invoice.tracking?.downloads || 0}</td></tr>
                    </table>
                </div>
            </div>
    ...
    async function sendInvoiceNow(invoiceId) {
        try {
            ...
            if (!confirm(`Envoyer la facture ${invoice.invoice_id} à ${invoice.client.email} ?`)) {
                return;
            }
            ...
            showNotification(`Facture ${invoice.invoice_id} envoyée avec succès`, 'success');
    ...
    function duplicateInvoice(invoiceId) {
        ...
        showNotification(`Facture dupliquée: ${duplicate.invoice_id}`, 'success');
    }
    ...
    async function sendReminders() {
        try {
            ...
            if (remindersSent > 0) {
                ...
                showNotification(`${remindersSent} relance(s) envoyée(s)`, 'success');
            } else {
                showNotification('Aucune relance à envoyer', 'info');
            }
    ...
    function bulkSendInvoices() {
        ...
        if (confirm(`Envoyer ${selectedIds.length} facture(s) sélectionnée(s) ?`)) {
    ...
    function bulkDuplicateInvoices() {
        ...
        if (confirm(`Dupliquer ${selectedIds.length} facture(s) sélectionnée(s) ?`)) {
    ...
    function exportInvoices() {
        try {
            const exportData = state.invoices.map(invoice => ({
                ...
                'Entité': config.entityTemplates[invoice.entity].name,
                'Date': formatDate(invoice.invoice_date),
                'Échéance': formatDate(invoice.due_date),
                'Montant TTC': invoice.total_ttc,
                'Statut': config.statuses[invoice.status],
                'Envoyée': invoice.sent_date ? formatDate(invoice.sent_date) : '',
                'Payée': invoice.paid_date ? formatDate(invoice.paid_date) : ''
            }));
    ...
    function renderTrackingInfo(tracking, status) {
        if (status === 'draft') {
            return '<span class="text-muted small">Non envoyée</span>';
        }
        
        if (!tracking || (!tracking.opens && !tracking.downloads)) {
            return '<span class="text-muted small">Aucune activité</span>';
        }
        
        return `
            <div class="tracking-info">
                <div class="small">
                    👁️ ${tracking.opens || 0} ouverture(s)
                </div>
                <div class="small">
                    📥 ${tracking.downloads || 0} téléchargement(s)
                </div>
                ${tracking.last_activity ? `<div class="text-muted small">Dernière: ${formatDate(tracking.last_activity)}</div>` : ''}
            </div>
        `;
    }
    ...
    function renderInvoiceActions(invoice) {
        ...
        // Action voir
        actions.push(`
            <button class="action-btn action-view" data-action="view" data-invoice-id="${invoice.invoice_id}" title="Voir détails">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
        ...
        // Action envoyer (si brouillon ou envoyée)
